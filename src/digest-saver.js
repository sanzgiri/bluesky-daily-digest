...
export class DigestSaver {
  ...
  async saveDigest(summary, metadata, costReport, postsCount) {
    if (!this.config.localSaving?.enabled) {
      console.log('ℹ️  Local digest saving is disabled');
      return;
    }
    ...
    try {
      ...
      console.log(`✓ Digest saved locally to: ${filepath}`);
      return filepath;
    } catch (error) {
      console.error('✗ Failed to save digest locally:', error.message);
      throw error;
    }
  }
  ...
  generateMarkdown(summary, metadata, costReport, postsCount, date) {
    ...
    let markdown = `# 🦋 Bluesky Daily Digest\n\n`;
    markdown += `**${dateString}**\n\n`;
    markdown += `---\n\n`;
    markdown += summary;
